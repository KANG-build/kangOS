...
BOOL kChangeKeyboardLED( BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn )
{
    ...
    // 키보드가 커맨드를 가져갔으므로 ACK가 올 때까지 대기
    for( j = 0 ; j < 100 ; j++ )
    {
        for( i = 0 ; i < 0xFFFF ; i++ )
        {
...
{
    // 숫자 패드나 확장 키 범위를 제외한 범위(스캔 코드 2~53)의 영문자가 아니면 숫자나 기호임
...
BOOL kIsUseCombinedCode( BYTE bScanCode )
{
    ...
    {
        // 만약 Shift 키와 Caps Lock 키 중에 하나만 눌려 있으면 조합된 키를 되돌려 줌
    ...
    {
        // Num Lock 키가 눌려 있으면 조합된 키를 되돌려 줌
...
void UpdateCombinationKeyStatusAndLED( BYTE bScanCode )
{
    ...
    // Num Lock 키의 스캔 코드(69)이면 Num Lock의 상태 갱신하고 LED 상태 변경
