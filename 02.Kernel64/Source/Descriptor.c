...
// 16바이트 크기의 GDT 엔트리에 값을 설정 (TSS 세그먼트 디스크립터)
...
void kInitializeIDTTables( void )
{
    ...
    // 0~IDT_MAXENTRYCOUNT-1까지 모두 DummyHandler로 연결
...
void kSetIDTEntry( IDTENTRY* pstEntry, void* pvHandler,
                   WORD wSelector, BYTE bIST,
                   BYTE bFlags, BYTE bType )
{
    pstEntry->wLowerBaseAddress = ( QWORD ) pvHandler & 0xFFFF;
    pstEntry->wSegmentSelector = wSelector;
    pstEntry->bIST = bIST & 0x3;
    pstEntry->bTypeAndFlags = bType | bFlags;
    pstEntry->wMiddleBaseAddress =
        ( ( QWORD ) pvHandler >> 16 ) & 0xFFFF;
